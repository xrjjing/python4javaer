...
- **integration_gateway_service**：使用 `httpx` 异步 HTTP 客户端调用后端服务，是网络编程的典型应用
- **03.项目实战/05_简单爬虫_新闻标题抓取**：使用 `requests` + `BeautifulSoup` 实现 HTTP 客户端抓取
- **frontend 与 RBAC API 交互**：`login.html`/`admin.html` 通过 `fetch()` 调用后端 REST API
- **综合练习**：参见 `15_进阶专题.md` 中的"审计日志分析与报表生成"，涉及 `requests.get()` 调用日志服务 API

**Java 对比**：
- Python `requests` ≈ Java `HttpClient` / OkHttp / RestTemplate
- Python `socket` ≈ Java `java.net.Socket`
- Python `http.server` ≈ Java 内嵌 Jetty / SimpleHttpServer

## 相关资源

- [Python官方文档 - socket](https://docs.python.org/zh-cn/3/library/socket.html)
- [Python官方文档 - http.server](https://docs.python.org/zh-cn/3/library/http.server.html)
