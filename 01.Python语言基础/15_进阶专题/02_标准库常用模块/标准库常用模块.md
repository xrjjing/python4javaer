...
```python
from itertools import groupby, accumulate, islice, pairwise, batched
...
list(pairwise([10, 12, 15]))  # [(10, 12), (12, 15)]

# batched - 等分迭代器块（Python 3.12+）
...
### 4.7 partial + lru_cache 组合

对带配置的纯函数，先 partial 固定参数，再缓存：

```python
from functools import partial, lru_cache

def fetch_page(url: str, timeout: float = 3.0) -> str:
    ...

cached_fetch = lru_cache(maxsize=256)(partial(fetch_page, timeout=5.0))
```
...
mac = hmac.new(b"secret", b"payload", hashlib.sha256).hexdigest()
```
