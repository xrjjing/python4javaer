...
## 扩展阅读：原生 DB-API 与 SQL 基础

本项目使用的是 SQLAlchemy ORM，如果你希望了解「更接近数据库驱动层」的写法，可以：

- 阅读 Python 官方文档中的「Python Database API Specification v2.0」章节，理解 DB-API 的统一接口约定；
- 对照本项目的 `app/database.py`，思考如果不用 ORM，如何直接使用连接对象执行 `SELECT / INSERT / UPDATE / DELETE`；
- 结合 `01.Python语言基础/15_进阶专题` 中与数据库、IO 相关的内容，自己尝试写一个使用原生 DB-API 操作 SQLite 的小脚本，再对比 ORM 版本的优缺点。
