...
## 九、2025-12 查漏补缺与文档/前端同步计划（新增）

- [x] 文档合并精简：创建 `docs/快速开始与依赖.md`、`docs/文档导航与精简说明.md`、`docs/查漏补缺清单.md`，并在 `README.md` 更新导航。前端页面未直接引用旧文档，已确认无需同步修改。
- [x] 高阶函数与 functools.partial 示例：新增 `demo_hof_partial.py` 与文档补充；`frontend/index.html` 已增加链接。
- [x] 面向对象高级（@property/__slots__/元类等）：新增 `advanced_oop.md`、`demo_oop_advanced.py`，并在前端导航补充链接。
- [x] 调试与性能分析专题：新增 `15_进阶专题/08_调试与性能分析` 文档与 demo，前端导航与缺口清单同步。
- [x] 标准库进阶补全：`02_标准库常用模块` 增加 contextlib/pairwise/batched/cached_property/hashlib/hmac/base64 等，并新增 `demo_stdlib_adv.py`；前端入口已同步。
- [x] IO 扩展：新增 `09_IO扩展/09_IO扩展.md` 与 `demo_io_ext.py`（StringIO/BytesIO/struct/gzip/zipfile），前端已同步。
- [x] 并发进阶：`高级并发技巧.md` + `demo_concurrency_adv.py`、`asyncio进阶.md` + `demo_asyncio_adv.py`；前端同步。
- [x] 网络客户端：`网络客户端进阶.md` + `demo_network_clients.py`（Mock 版 urllib/requests/httpx），前端同步。
- [x] Mock 离线能力：新增 tests/fixtures（fake MySQL/Redis/MinIO）、`.env.example` 占位；portal 增 Mock 入口；提醒前端 README 中的 mock 模式。
- [x] 覆盖率提升：新增脚本单测（rename_files、log_analyzer、archive_old_logs、process_metrics）；全量 pytest 通过。
- [ ] 数据库与虚拟环境工具对比：在 `docs/快速开始与依赖.md` 增加 pipx/uv/pyenv 对比小节；对应前端导航更新。

> 执行要求：每个缺口项需同时补“文档 + 可运行示例代码 + README/前端导航链接”，完成后在此清单勾选。
