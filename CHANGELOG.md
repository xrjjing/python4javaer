...
- 牛牛待办：全新待办事项管理应用
  ...
  - 任务状态流转：未开始 → 进行中 → 已完成
  - 四级优先级：紧急(红)/高(蓝)/中(橙)/低(灰)
  - 四象限视图：重要紧急矩阵，支持任务分配到象限
  - 列表视图：按状态分组的任务列表
  - 看板视图：三栏拖拽（未开始/进行中/已完成）
  - 日历视图：月历显示任务截止日期
  - 分类管理：自定义分类（图标+颜色）
  - 番茄钟：25分钟专注计时，番茄记录统计
  - 便签悬浮窗：今日任务快速查看，可拖拽/透明度调节
  - 工作总结：日报/周报/月报统计，一键复制
  - 数据导入导出：JSON 格式备份与恢复
  - 键盘快捷键：N新建、E编辑、Space完成、1-4切换视图
  - 9种主题：light/cute/office/neon-light/forest/sunset/dark/neon/cyberpunk
  - 单元测试：28个测试用例覆盖核心业务逻辑
  - 应用图标：可爱牛牛风格 SVG 图标，支持转换为 ICO/ICNS

### Fixed
- 牛牛待办：番茄钟完成后正确调用后端 API 持久化记录
- 牛牛待办：工作总结统计字段名与后端对齐（pomodoro_count/pomodoro_hours）

### Changed
- 狗狗百宝箱：工具箱导航重构为四大分类（编码转换/加密安全/文本处理/开发工具）
- 狗狗百宝箱：节点管理默认收起
- 狗狗百宝箱：JSON 格式化工具栏布局优化，缩进选项移至输入区
- 狗狗百宝箱：数据格式转换布局优化，配置项移至底部
- 狗狗百宝箱：颜色转换输出标签宽度修复
- 狗狗百宝箱：SQL 格式化按钮样式修复
- 狗狗百宝箱：Cron 预设按钮添加 title 提示
- 牛牛待办：顶部工具栏按钮添加文字标注（设置/统计/总结/便签）
- 牛牛待办：吉祥物颜色跟随主题变化（全9个主题适配）
- 牛牛待办：新增成就系统
  - 5大成就类别：任务达人、专注大师、连续打卡、早起鸟儿、夜猫子
  - 4级成就等级：铜牌、银牌、金牌、钻石
  - 成就殿堂展示页面，显示进度和解锁状态
  - 成就解锁动画提示
  - 自动检测并解锁成就（完成任务/番茄钟时触发）
...
- 狗狗百宝箱：新增数据格式转换工具（M18）
  - JSON ↔ YAML 双向转换（依赖 js-yaml CDN）
  - JSON ↔ XML 双向转换（依赖 fast-xml-parser CDN）
  - 自动检测输入格式（JSON/YAML/XML）
  - 支持配置 XML 根元素名称和 JSON 缩进
  - 内置降级 XML 生成器，CDN 不可用时仍可使用
- 狗狗百宝箱：JSON 格式化工具新增树形视图（M16）
  - 文本/树形视图切换
  - 可展开/折叠的树形结构
  - 全部展开/折叠按钮
  - 类型高亮：字符串(绿)、数字(蓝)、布尔(紫)、null(灰)
- 狗狗百宝箱：新增文本处理工具（M11）
  - 去重：支持区分大小写和忽略首尾空白选项
